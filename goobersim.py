...
def main():
    ...
    while True:
        ...
        for i in range(100):
            ...
            keys = pygame.key.get_pressed()
            if keys[pygame.K_j]:
                strafe_angle = person_rotation + 90  # Calculate the angle perpendicular to the current rotation
                x_strafe, z_strafe = xyrotation(strafe_angle, 0.1 + movspeed / 3)  # Calculate strafing movement
                new_person_x = person_x + x_strafe
                new_person_z = person_z + z_strafe
                if -floor_size < new_person_x < floor_size and -floor_size < new_person_z < floor_size:
                    person_x = new_person_x
                    person_z = new_person_z
